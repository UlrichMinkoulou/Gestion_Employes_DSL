        ...
        if(sqlite3_step(stmt) == SQLITE_DONE) 
            {
                if(sqlite3_changes(m_db) > 0)
                    cout << ANSI_BOLD << ANSI_GREEN << "\n[PASS]     "<< ANSI_RESET << "Mise a jour du mot de passe reussie !" << endl;
                else    
                    cout << ANSI_BOLD << ANSI_RED << "\n[FAIL]   " << ANSI_RESET << "Aucun Employe trouve avec cet Id !" << endl;
            }
        else
            {
                cerr << ANSI_BOLD << ANSI_RED << "\n[FAIL]  " << ANSI_RESET << "Erreur lors de la mise a jour du mot de passe : " << sqlite3_errmsg(m_db) << endl;
            }
            ...
            if(m_data.res == true)
            {
                        ...
                        if(sqlite3_step(stmt.get()) == SQLITE_ROW)
                        {
                            int etat = (sqlite3_column_int(stmt.get(), 12)); //Récupère la valeur de la colonne ETAT
                                ...
                                if(etat ==0) {
                                        m_data.res = false;
                                        std::cout << ANSI_BOLD << ANSI_RED<< "[FAIL]  " << ANSI_RESET << "Compte inactif, veillez contacter les RH!\n\n"  << std::endl;
                                        // sqlite3_finalize(stmt);
 ...
 // const vector<EmployeData> vect = m_caheEmployes;
    ...
    void DataBase::envoyer_MSG(string destinataire, string expediteur, string contenu, string objet)
    {
        ...
        if(destinataire[0] == 'E')
        {
            ...
            if(valide == true)
            {
                cout << ANSI_BOLD << ANSI_GREEN << "\n[PASS]   "<< ANSI_RESET << "Id Destinataire Employe valide !" << endl;
            }
            else{
                    cerr << ANSI_BOLD << ANSI_RED << "\n[FAIL]     " << ANSI_RESET << "Id Destinataire Employe invalide !" << endl;
        ...
        } else if(destinataire[0] == 'A')
            {
                ...
                if(valide == true)
                {
                    ...
                    cerr << ANSI_BOLD << ANSI_RED << "\n[FAIL]   " << ANSI_RESET << "Id Destinataire Administrateur invalide !" << endl;
                    std::cout << "\n[Appuyer sur une touche pour continuer...]" << std::endl;
                    std::cin.ignore();
                    std::cin.get();
                    return;
                ...
                {
                    cerr << ANSI_BOLD << ANSI_RED << "\n[FAIL]  " << ANSI_RESET << "Identifiant de destinataire invalide !" << endl;
                    return;
                }
        ...
        if(sqlite3_prepare_v2(m_db, sql.c_str(), -1, &stmt, NULL) != SQLITE_OK)
            {
                cerr << ANSI_BOLD << ANSI_RED << "\n[FAIL]  " << ANSI_RESET << "Erreur de preparation : " << sqlite3_errmsg(m_db) << endl;
                return;
            }
        ...
        {
            // cout  << " " << endl;
            cout << ANSI_BOLD << ANSI_GREEN << "\n[PASS]  "<< ANSI_RESET << "Message envoye avec SUCCES !!" << endl;
        }
        else 
            cerr << ANSI_BOLD << ANSI_RED << "\n[FAIL]  " << ANSI_RESET << "Erreur d'envoi : " << sqlite3_errmsg(m_db) << ANSI_RESET << endl;
